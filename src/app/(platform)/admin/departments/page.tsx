  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Departments & Semesters</h1>
          <p className="text-slate-500 dark:text-slate-400 mt-1">Manage the academic structure of your university.</p>
        </div>
      ...
      {academicYears.length === 0 ? (
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-8 text-center">
          ...
          <h3 className="text-lg font-medium text-slate-900 dark:text-white mb-1">No Departments Setup</h3>
          <p className="text-slate-500 mb-4">Start by adding an academic year and creating departments.</p>
                              ...
                              {cls.semesters && cls.semesters.length > 0 ? (
                                cls.semesters.map((sec: any) => (
                                  <div key={sec.id} className="flex items-center justify-between text-sm">
                                    <span className="text-slate-600 dark:text-slate-300">Semester {sec.name}</span>
                                ...
                                <p className="text-sm text-slate-500 italic">No semesters created yet.</p>
                              )}
