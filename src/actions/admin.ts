...
export async function inviteUser(formData: FormData) {
  ...
  await adminClient.from('school_memberships').insert({ user_id: newUser.id, school_id: schoolId, role });
...
export async function resolveReport(reportId: string, action: string, notes: string) {
  ...
  const { data: report } = await supabase.from('reports').select('school_id').eq('id', reportId).single();
